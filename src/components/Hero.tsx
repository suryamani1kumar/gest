...
const slides = [
  ...
  },
  {
    ...
    features: [
      {
        title: "Brings Peace & Calm",
        icon: "/images/banner/icon/meditation.png",
      },
      {
        title: "Protects From Negativity",
        icon: "/images/banner/icon/shield.png",
      },
      {
        title: "Enhances Focus & Clarity",
        icon: "/images/banner/icon/lotus.png",
      },
      {
        title: "Supports Spiritual Growth",
    ...
    <>
      <section className="relative h-[calc(100vh-116px)] md:h-[calc(100vh-130px)] w-full overflow-hidden">
        <Swiper
          ...
          className="h-[calc(100vh-116px)] md:h-[calc(100vh-130px)] w-full"
        >
            ...
            <SwiperSlide key={index}>
              <div className="relative h-[calc(100vh-116px)] md:h-[calc(100vh-130px)] w-full">
                ...
                <div className="relative z-10 flex h-full items-center">
                  <div className="mx-auto grid max-w-7xl grid-cols-1 lg:grid-cols-2 gap-10 px-6 lg:px-10">
                    {/* Left */}
                    <div className="flex flex-col justify-center text-white">
                      <span className="mb-4 text-sm md:text-lg tracking-[5px] uppercase text-amber-300">
                        {slide.subtitle}
                      </span>

                      <h1 className="font-serif text-5xl md:text-7xl font-bold text-amber-100">
                        {slide.title}
                      </h1>

                      <div className="my-5 h-px w-48 bg-amber-500" />

                      <p className="max-w-xl text-base md:text-xl leading-9 text-neutral-200">
                        {slide.description}
                      </p>
                      {/* Button */}
                      <div className="mt-10">
                        <button className="rounded-lg bg-amber-400 px-8 py-4 text-lg font-semibold text-black transition hover:bg-amber-300">
                          Explore Collection →
                        </button>
                      </div>
                      {/* Features */}
                      <div className="mt-10 grid grid-cols-2 gap-2 md:grid-cols-4">
                        {slide.features.map((item, index) => (
                          <div key={index} className="text-center w-[100px]">
                            ...
                            <p className="mt-3 text-xs font-medium uppercase tracking-wide text-white">
                              {item.title}
                            </p>
                          </div>
                        ))}
